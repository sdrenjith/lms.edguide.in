...
>
    @if (false)
        <span
            class="fi-sidebar-item-button relative flex items-center justify-center gap-x-3 rounded-lg px-2 py-2 outline-none transition duration-75 text-gray-400 dark:text-gray-500 cursor-not-allowed opacity-60 custom-tooltip-wrapper"
        ...
        >
            @if (filled($icon) && ((! $subGrouped) || $sidebarCollapsible))
                ...
                />
            @endif
            <span class="fi-sidebar-item-label flex-1 truncate text-sm font-medium">{{ $slot }}</span>
            <span class="custom-tooltip-text">Video option is temporarily disabled</span>
        </span>
    @else
        <a
