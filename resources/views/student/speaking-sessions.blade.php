...
<head>
    ...
    <title>Live Classes - EdGuide</title>
        ...
        <div class="flex-1 bg-white">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div class="mb-8">
                    ...
                    <p class="mt-2 text-gray-600">Schedule and manage your speaking practice sessions</p>
                </div>
                
                <!-- Live Classes Content -->
                <div class="space-y-6">
                    @if($speakingSessions->count() > 0)
                        @foreach($speakingSessions as $session)
                            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-4">
                                        <div class="flex-shrink-0">
                                            <div class="h-12 w-12 bg-[#14b8a6] rounded-full flex items-center justify-center">
                                        ...
                                        <div>
                                            <h3 class="text-lg font-medium text-gray-900">{{ $session->description ?? 'Speaking Session' }}</h3>
                                            <p class="text-sm text-gray-500">
                                                Scheduled for {{ \Carbon\Carbon::parse($session->session_date)->format('M d, Y') }} at {{ \Carbon\Carbon::createFromFormat('H:i:s', $session->session_time)->format('h:i A') }}
                                            </p>
                                            @if($session->gmeet_link)
                                                <p class="text-sm text-blue-600 mt-1">
                                                    <a href="{{ $session->gmeet_link }}" target="_blank" class="hover:underline">
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900">No speaking sessions scheduled</h3>
                            <p class="mt-1 text-sm text-gray-500">Contact your instructor to schedule a speaking session.</p>
                        </div>
                    @endif
                </div>
            </div>
