...
<form action="{{ route('admin.courses.store') }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <select class="form-select" id="days" name="days[]" multiple size="6">
            @foreach($days as $day)
                <option value="{{ $day->id }}">Day {{ $day->day_number }}: {{ $day->title ?? 'Untitled' }}</option>
            @endforeach
        </select>
        @error('days')<div class="text-danger small">{{ $message }}</div>@enderror
    </div>
    <input type="hidden" name="order[]" id="daysOrderInput">
    <button type="submit" class="btn btn-primary btn-animated"><i class="bi bi-check-circle"></i> Create Course</button>
</form>
@endsection
@push('scripts')
<script>
